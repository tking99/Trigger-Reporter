...
class MonitoringResultsTable:
    ...
    def __init__(self, report_data):
        ...
        with PdfPages(f'{self.report_data.heading.site.name}_{self.report_data.heading.name}_{self.report_date}_{self.RESULTS_TYPE}.pdf') as self.pdf:
            ...
            for num, array_data in enumerate(self.report_data.array_data):
                if num+1 %  self.ARRAYS_PER_PAGE == 0:
                    #new page
                    self.plt_save_close()
                    self.plt_setup()
                    
                self.create_table(array_data, num)
    ...
    def create_table(self, array_data, num):
        array = array_data.array 
        num_rows =  4 #math.ceil(len(self.report_data.arrays) / self.ARRAYS_PER_ROW)
        axTable = plt.subplot(num_rows,self.ARRAYS_PER_ROW, num+1, frameon =False)
